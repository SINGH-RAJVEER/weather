...
import { AlertTriangle, ArrowLeft, Camera, Loader, MapPin, Send } from "lucide-react-native";
// ReportForm.tsx
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <View className="flex-row items-center mb-6">
          ...
          <View className="flex-1">
            <Text className="text-xl font-bold text-neutral-900">Report Ocean Hazard</Text>
        ...
        {/* Hazard Type */}
        <Text className="text-sm font-medium text-neutral-700 mb-2">Hazard Type</Text>
        <View className="flex-row flex-wrap mb-4">
          {Object.entries(hazardTypeLabels).map(([key, label]) => (
            ...
            >
              <Text
                className={`text-sm ${
                  formData.type === key ? "text-white font-medium" : "text-neutral-700"
                }`}
              >
                {label}
              </Text>
        ...
        {/* Severity */}
        <Text className="text-sm font-medium text-neutral-700 mb-2">Severity Level</Text>
        <View className="flex-row flex-wrap mb-4">
          {Object.entries(severityLabels).map(([key, label]) => (
            <TouchableOpacity
              ...
              className={`px-4 py-2 rounded-lg border mr-2 mb-2 ${
                formData.severity === key ? "bg-sky-500 border-sky-600" : getLevelColor(key)
              }`}
            >
              <Text
                className={`text-sm font-medium ${formData.severity === key ? "text-white" : ""}`}
              >
                {label}
              </Text>
            </TouchableOpacity>
        ...
        {/* Description */}
        <Text className="text-sm font-medium text-neutral-700 mb-2">Detailed Description</Text>
        <TextInput
        ...
        {/* Location */}
        <Text className="text-sm font-medium text-neutral-700 mb-2">Location</Text>
        ...
        {/* File Upload */}
        <Text className="text-sm font-medium text-neutral-700 mb-2">Photos/Videos (Optional)</Text>
        <TouchableOpacity
          onPress={pickFiles}
          className="border-2 border-dashed border-neutral-300 rounded-lg p-6 items-center mb-2"
        >
          <Camera size={32} color="#9ca3af" />
          <Text className="text-sm text-neutral-600 mt-2">Tap to upload photos or videos</Text>
        </TouchableOpacity>
        ...
        >
          {isSubmitting ? (
            ...
            <>
              ...
              <Text className="ml-2 text-white font-medium">Submit Hazard Report</Text>
            </>
          )}
        </TouchableOpacity>
      </ScrollView>
