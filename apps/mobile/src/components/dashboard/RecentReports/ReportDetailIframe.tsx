        ...
        <View className="bg-white rounded-2xl w-[90%] max-h-[85%] overflow-hidden">
          ...
          >
            ...
            <View className="flex-row items-center mb-4">
              ...
              <View className={`ml-2 px-2 py-1 rounded-full border ${colors.border}`}>
                <Text className={`text-xs font-medium ${colors.text}`}>
                  {report.severity.charAt(0).toUpperCase() + report.severity.slice(1)}
                </Text>
              </View>
              {report.verified && <CheckCircle2 size={18} color="green" className="ml-2" />}
            </View>
            ...
            {/* Description */}
            <Text className="text-base text-neutral-700 mb-4">{report.description}</Text>

            {/* Meta Info */}
            <View className="mb-4">
              <View className="flex-row items-center mb-2">
                <MapPin size={14} color="#64748b" />
                <Text className="ml-1 text-sm text-neutral-500">{report.location.address}</Text>
              </View>
            ...
            <View className="flex-row justify-between border-t border-neutral-200 pt-3">
              <View className="flex-row items-center">
                ...
                <Text className="ml-1 text-sm text-neutral-500">{report.likes}</Text>
              </View>
              <View className="flex-row items-center">
                <MessageCircle size={14} color="#64748b" />
                <Text className="ml-1 text-sm text-neutral-500">{report.comments}</Text>
              </View>
            </View>
          </ScrollView>
        </View>
