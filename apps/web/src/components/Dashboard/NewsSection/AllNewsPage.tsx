import type { NewsArticle } from "@weather/types";
import {
  AlertCircle,
  AlertTriangle,
  Clock,
...
const AllNewsPage: React.FC<AllNewsPageProps> = ({ articles, isLoading, onBack }) => {
  ...
  const getCategoryLabel = (category: string) => {
    const categoryData = categories.find((cat) => cat.id === category);
    return categoryData?.label || category.charAt(0).toUpperCase() + category.slice(1);
  };
  ...
  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto">
        <h2 className="text-xl font-semibold text-neutral-900 mb-4">{t("news:allNews")}</h2>
    ...
    <div className="hazard-card max-w-6xl mx-auto bg-white text-neutral-900 p-6 text-lg">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-neutral-900">{t("news:allNews")}</h3>
          </div>
        </div>
        ...
        {categories.map((category) => {
          ...
          return (
            ...
            >
              ...
              <span className="card-bg px-2 py-0.5 rounded-full text-xs">{count}</span>
            </button>
          );
        })}
      </div>
            ...
            >
              <div className="flex">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-neutral-600 mb-3 line-clamp-3">{article.summary}</p>
