...
export const LocationSummary: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const translateAction = (action: string) => {
    ...
    const actionMappings: Record<string, string> = {
      "Evacuate low-lying areas immediately": "dashboard:actions.evacuatelowlyingareasimmediately",
      ...
      "Exercise caution near the shoreline": "dashboard:actions.exercisecautionneartheshoreline",
      "Secure loose outdoor items and vessels":
        "dashboard:actions.securelooseoutdooritemsandvessels",
      "No immediate action; stay informed": "dashboard:actions.noimmediateactionstayinformed",
  ...
  const translateHazardType = (hazardType: string) => {
    ...
    return translated === translationKey ? hazardType.replace(/_/g, " ") : translated;
  };
  ...
  return (
    <div className={`hazard-card p-8 ${className || "w-full"}`}>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-neutral-900">{title}</h1>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="p-4 bg-white rounded-lg shadow-sm">
          <h3 className="text-lg font-semibold text-neutral-900">{t("dashboard:summary")}</h3>
          <p className="text-neutral-700 mt-2">
            {(locationDetails?.headline && (
              <strong className="block text-neutral-900">{locationDetails?.headline}</strong>
            )) ||
              null}
          ...
          <h4 className="mt-4 font-semibold">{t("dashboard:recommendedActions")}</h4>
          <ul className="list-disc list-inside text-sm text-neutral-700 mt-2">
            {(locationDetails?.actions || insights.recommendedActions).map((a, i) => (
