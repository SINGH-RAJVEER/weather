...
import type { AnalystReport } from "./OfficialReportsView";
...
const IssueAdvisoryPage: React.FC<IssueAdvisoryPageProps> = ({ reportData, onBack }) => {
  const { user } = useAuth();
  const [title, setTitle] = useState("");
  const [content, setContent] = useState("");
  const [severityLevel, setSeverityLevel] = useState<"info" | "warning" | "emergency">("info");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitMessage(`Error issuing advisory: ${errorData.message || response.statusText}`);
      }
    } catch (error) {
      setSubmitMessage(`Network error: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
  ...
  return (
    <div className="hazard-card p-6">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-neutral-900">Issue Public Advisory</h2>
      ...
      {reportData && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
          ...
          <p>
            <strong>Affected Regions:</strong> {reportData.affectedRegions.join(", ")}
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-neutral-700 mb-2">Advisory Title</label>
          <input
            type="text"
            value={title}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select
              value={severityLevel}
              onChange={(e) => setSeverityLevel(e.target.value as "info" | "warning" | "emergency")}
              className="w-full px-4 py-3 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-colors duration-200"
            >
