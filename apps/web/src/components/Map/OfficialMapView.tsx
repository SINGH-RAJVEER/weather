...
const LiveRiskScore: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (toggles.heatmap) {
      ...
      hazard.forEach((r) => {
        if (
          ...
          typeof r.coordinates.lon !== "number"
        )
          return;
    ...
    if (toggles.swellDots) {
      const grp = L.layerGroup();
      hazard
        .filter((h) => typeof h.hazardType === "string" && /swell/i.test(h.hazardType))
        .forEach((r) => {
          if (
            !r.coordinates ||
            typeof r.coordinates.lat !== "number" ||
    ...
    if (toggles.swellHeat) {
      const swellReports = hazard.filter(
        (h) => typeof h.hazardType === "string" && /swell/i.test(h.hazardType)
      );
      ...
      swellReports.forEach((r) => {
        ...
        for (let i = 0; i < num; i++) {
          ...
          swellPoints.push([
            r.coordinates.lat + latOffset,
            r.coordinates.lon + lonOffset,
            intensity,
          ]);
        }
      });
  ...
  return (
    ...
    >
      ...
      {isMenuOpen && (
        <div className="fixed right-0 top-16 h-[calc(100vh-4rem)] z-[2200] w-64 bg-white/70 dark:bg-gray-900/70 border-l border-gray-200 dark:border-gray-800 shadow-xl flex flex-col backdrop-blur-md transition-all duration-300">
