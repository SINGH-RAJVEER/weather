  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (showLightbox && report?.media && report.media.length > 1) {
        if (e.key === "ArrowRight")
          setCurrentIndex((i) => (i + 1) % report.media!.length);
        if (e.key === "ArrowLeft")
          setCurrentIndex(
            (i) => (i - 1 + report.media!.length) % report.media!.length
          );
      }
              ...
              <>
                <button
                  ...
                  onClick={() =>
                    setCurrentIndex(
                      (i) =>
                        (i - 1 + report.media!.length) % report.media!.length
                    )
                  }
                >
                  ‹
