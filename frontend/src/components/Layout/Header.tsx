...
export const Header: React.FC<HeaderProps> = ({ onNavigate, currentPage }) => {
  ...
  const { selectedLocation, setSelectedLocation, availableLocations } =
    useLocation();
  ...
  return (
    <>
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-2 w-48 card-bg rounded-lg shadow-lg border border-neutral-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-1 group-hover:translate-y-0 z-60">
                    <div className="py-2">
                      ...
                      <button onClick={() => onNavigate("edit-profile")} className="w-full px-4 py-2 text-left text-sm text-neutral-700 hover:bg-neutral-50 flex items-center space-x-2">
      ...
      {user?.role === "citizen" && currentPage === "dashboard" && <RollingStats stats={stats} isLoading={isStatsLoading} />}
    </>
  );
};
