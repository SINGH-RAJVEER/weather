...
import type { PublicAdvisory } from "../types/types";
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    return advisories
      ...
      .filter((a) =>
        a.targetRegions.some((r: string) => {
          ...
          return (
            loc.includes(nr) ||
            nr.includes(loc.split(",")[0]) ||
            loc.split(",")[0].includes(nr)
          );
        })
      )
  ...
  const getLocationDetails = (location: string) => {
    const key = knownLocations.find((k) =>
    ...
    const headline = i18n.exists(headlineKey)
      ? (i18n.t(headlineKey) as string)
      : undefined;
  ...
  const getLocationInsights = (location: string) => {
    const matchedReports = reports.filter(
      (r) =>
        ...
        location
          .toLowerCase()
          .includes(r.location.address?.split(",")[0].toLowerCase())
    );
    ...
    const hotspot = hotspots.find((h) =>
      matchedReports.some(
        (mr) =>
          mr.location.lat === h.location.lat &&
          mr.location.lng === h.location.lng
      )
    );
