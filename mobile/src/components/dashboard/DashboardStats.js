    ...
    return (
      <View className="flex-row flex-wrap justify-between mb-8">
        {[...Array(4)].map((_, i) => (
          <View
            key={i}
            className="w-[48%] h-24 bg-neutral-200 rounded-lg mb-4 animate-pulse"
          />
        ))}
      </View>
    );
    ...
    <View className="flex-row flex-wrap justify-between mb-8">
      {statCards.map((stat, index) => {
        ...
        return (
          <View
            key={index}
            className="w-[48%] bg-white rounded-xl p-4 mb-4 shadow"
          >
            {stat.isLive && (
            ...
            <View className="flex-row justify-between items-center">
              ...
              <View
                className={`w-12 h-12 ${stat.bgColor} rounded-lg items-center justify-center`}
              >
